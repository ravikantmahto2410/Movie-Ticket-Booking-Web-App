...
//Inngest function to cancel booking and release seats of show after 10 minutes of booking created if not made
    ...
    async({event, step}) => {
        ...
        await sendEmail({
            ...
            body: `<div style="font-family: Arial, sans-serif; line-height:1.5;">
                ...
                <p>
                    <strong>Date:</strong>${new Date(booking.show.showDateTime).toLocaleDateString('en-US', {timeZone:
                    'Asia/Kolkata'})}<br/>
                    <strong>Time:</strong> ${new Date(booking.show.showDateTime).toLocaleTimeString('en-US',{timeZone:
                    'Asia/Kolkata'})}
                </p>
                <p>Enjoy the show!</p>
                <p>Thanks for booking with us!<br/>--Entertaino Team</p>
            </div>`
        })
    }
...
const sendShowReminders = inngest.createFunction(
    ...
    async ({step}) => {
        ...
        //send reminder email
        const results = await step.run('send-all-reminders',
            async () => {
                return await Promise.allSettled(
                    reminderTasks.map(task => sendEmail({
                        ...
                        body: `<div style="font-family: Arial, sans-serif; padding:20px;">
                                    ...
                                    <br/>
                                    <p>Enjoy the show! <br/> Entertaino Team</p>
                               </div>`
                    }))
                )
            }
    ...
    async ({ event }) => {
        ...
        for(const user of users){
            ...
            const body = `<div style="font-family": Arial, sans-serif; padding: 20px;">
                <h2>Hi ${userName}</h2>
                <p>We have just added a new show to your library: </p>
                <h3 style="color: #F84565;">"${movieTitle}"</h3>
                <p>Visit our website</p>
                <br/>
                <p>Thanks,<br/> Quickshow</p>
            </div>`;
